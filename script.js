...
function validateForm() {
  ...
  var nameErr = (nimErr = emailErr = mobileErr = agamaErr = prodiErr = alamatErr = genderErr = true);
  ...
  if (mobile == '') {
    printError('mobileErr', 'Tolong masukkan nomor telepon anda');
    ...
    if (regex.test(mobile) === false) {
      printError('mobileErr', 'Masukkan nomor yang valid');
      var elem = document.getElementById('mobile');
      elem.classList.add('input-2');
      elem.classList.remove('input-1');
    } else {
  ...
  if ((nameErr || nimErr || emailErr || mobileErr || agamaErr || prodiErr || alamatErr || genderErr) == true) {
    return false;
  }
}
